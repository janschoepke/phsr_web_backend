...
abstract class Victim implements ActiveRecordInterface
{
    ...
    protected function doAddMailing(ChildMailing $mailing)
    {
        ...
        $victimMailings->setMailing($mailing);
        ...
        // set the back reference to this object directly as using provided method either results
        // in endless loop or in multiple relations
        if (!$mailing->isVictimsLoaded()) {
            $mailing->initVictims();
            $mailing->getVictims()->push($this);
        } elseif (!$mailing->getVictims()->contains($this)) {
            $mailing->getVictims()->push($this);
        }
    ...
    public function removeMailing(ChildMailing $mailing)
    {
        if ($this->getMailings()->contains($mailing)) {
            ...
            if ($mailing->isVictimsLoaded()) {
                //remove the back reference if available
                $mailing->getVictims()->removeObject($this);
            }
