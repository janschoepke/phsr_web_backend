...
abstract class VictimMailingsQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$victim_id, $mailing_id] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = VictimMailingsTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
    ...
    {
        $sql = 'SELECT victim_id, mailing_id, timestamp, opened, clicked, customParam FROM Victim_Mailings WHERE victim_id = :p0 AND mailing_id = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            VictimMailingsTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(VictimMailingsTableMap::COL_VICTIM_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(VictimMailingsTableMap::COL_MAILING_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(VictimMailingsTableMap::COL_VICTIM_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(VictimMailingsTableMap::COL_MAILING_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    {
        if ($victimMailings) {
            $this->addCond('pruneCond0', $this->getAliasedColName(VictimMailingsTableMap::COL_VICTIM_ID), $victimMailings->getVictimId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(VictimMailingsTableMap::COL_MAILING_ID), $victimMailings->getMailingId(), Criteria::NOT_EQUAL);
