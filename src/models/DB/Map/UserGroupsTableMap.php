...
class UserGroupsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'DB.Map.UserGroupsTableMap';
     ...
     */
    const OM_CLASS = '\\DB\\UserGroups';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'DB.UserGroups';
     ...
     */
    const COL_USER_ID = 'User_Groups.user_id';

    /**
     * the column name for the group_id field
     */
    const COL_GROUP_ID = 'User_Groups.group_id';
    ...
    protected static $fieldNames = array (
        ...
        self::TYPE_COLNAME       => array(UserGroupsTableMap::COL_USER_ID, UserGroupsTableMap::COL_GROUP_ID, ),
    ...
    protected static $fieldKeys = array (
        ...
        self::TYPE_COLNAME       => array(UserGroupsTableMap::COL_USER_ID => 0, UserGroupsTableMap::COL_GROUP_ID => 1, ),
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\DB\\UserGroups');
        $this->setPackage('DB');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \DB\UserGroups $obj A \DB\UserGroups object.
     ...
     *
     * @param mixed $value A \DB\UserGroups object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \DB\UserGroups) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \DB\UserGroups object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(UserGroupsTableMap::COL_USER_ID);
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(UserGroupsTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(UserGroupsTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new UserGroupsTableMap());
        }
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \DB\UserGroups) { // it's a model object
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(UserGroupsTableMap::COL_USER_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(UserGroupsTableMap::COL_GROUP_ID, $value[1]));
...
//
UserGroupsTableMap::buildTableMap();
