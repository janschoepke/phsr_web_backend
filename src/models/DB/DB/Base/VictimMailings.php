...
abstract class VictimMailings implements ActiveRecordInterface
{
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[VictimMailingsTableMap::COL_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : VictimMailingsTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : VictimMailingsTableMap::translateFieldName('VictimId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->victim_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : VictimMailingsTableMap::translateFieldName('MailingId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->mailing_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : VictimMailingsTableMap::translateFieldName('Timestamp', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->timestamp = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : VictimMailingsTableMap::translateFieldName('Opened', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(VictimMailingsTableMap::COL_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getVictimId(),
            $keys[2] => $this->getMailingId(),
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setVictimId($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setMailingId($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setTimestamp($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setOpened($arr[$keys[4]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(VictimMailingsTableMap::COL_ID)) {
            $criteria->add(VictimMailingsTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(VictimMailingsTableMap::COL_VICTIM_ID)) {
